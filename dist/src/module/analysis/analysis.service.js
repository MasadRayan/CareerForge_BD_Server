...
const FREE_TIER_ANALYSIS_LIMIT = Number(process.env.FREE_TIER_ANALYSIS_LIMIT || 5);
...
const callGroqForAts = async (cvText, jdText) => {
    let raw;
    try {
...
const loadOrInitQuota = async (userId) => {
    const now = new Date();
...
const createAnalysisInDB = async (userId, payload) => {
    ...
    if (user?.role === "free_user") {
        ...
        if (quota.analyses_used_this_month >= FREE_TIER_ANALYSIS_LIMIT) {
            throw new AppError(`Free tier limit reached (${FREE_TIER_ANALYSIS_LIMIT} analyses/month). Upgrade to premium for unlimited analyses.`, 429);
        }
    }
...
const getAllAnalysesFromDB = async (userId) => {
...
const getAnalysisFromDB = async (userId, id) => {
...
const deleteAnalysisFromDB = async (userId, id) => {
