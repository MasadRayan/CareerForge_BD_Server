...
import { roadmapResponseSchema, } from "./roadmap.interface.js";
...
const callGroqForRoadmap = async (gapSkills, durationWeeks) => {
    if (durationWeeks <= 4) {
...
const callGroqSingleBatch = async (gapSkills, startWeek, endWeek, totalWeeks) => {
    const prompt = startWeek === 1 && endWeek === totalWeeks
        ? roadmapPrompt(gapSkills, totalWeeks)
        : roadmapBatchPrompt(gapSkills, startWeek, endWeek, totalWeeks);
    let raw;
    try {
        raw = await groqChatCompletion([{ role: "user", content: prompt }], { temperature: 0.4, maxTokens: 4096 });
    }
    catch (error) {
        await prisma.systemLogs
            ...
            .catch(() => { });
        throw new AppError("AI service is unavailable. Please try again in a moment.", 502);
    }
    ...
    if (!validated.success) {
        await prisma.systemLogs
            ...
            .catch(() => { });
        throw new AppError("AI returned an unexpected response shape. Please try again.", 502);
    }
    return validated.data;
};
const computeDurationWeeks = (interviewDate, fallback = 4) => {
    if (!interviewDate)
        return fallback;
    const now = new Date();
    const diffDays = Math.ceil((interviewDate.getTime() - now.getTime()) / DAY_IN_MS);
    if (diffDays <= 0)
        return fallback;
    const weeks = Math.ceil(diffDays / 7);
    return Math.min(Math.max(weeks, 1), 12);
};
const weekDateRange = (weekNumber) => {
...
const createRoadmapInDB = async (userId, payload) => {
    ...
    const durationWeeks = duration_weeks ?? computeDurationWeeks(analysis.jd.interview_date);
    const aiResult = await callGroqForRoadmap(analysis.gap_skills, durationWeeks);
...
const getAllRoadmapsFromDB = async (userId) => {
...
const getRoadmapFromDB = async (userId, id) => {
...
const completeTaskInDB = async (userId, roadmapId, taskId) => {
...
const updateRoadmapStatusInDB = async (userId, id, payload) => {
...
const deleteRoadmapFromDB = async (userId, id) => {
    const existing = await prisma.roadmaps.findFirst({
        where: { id, user_id: userId },
        select: { id: true },
    });
