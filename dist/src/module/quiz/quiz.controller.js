...
const getQuestions = async (req, res, next) => {
    try {
        const params = {
            role_category: req.query.role_category,
...
const submitAttempt = async (req, res, next) => {
    try {
        ...
        sendResponse(res, 200, true, result.is_correct
            ? "Correct! Well done."
            : `Incorrect. The correct answer was "${result.correct_answer}".`, result);
    }
    catch (error) {
        next(error);
    }
...
const getHistory = async (req, res, next) => {
    try {
        ...
        sendResponse(res, 200, true, "Attempt history fetched successfully", history);
    }
    catch (error) {
        next(error);
    }
