...
const getQuestionsFromDB = async (params) => {
    ...
    if (role_category &&
        !VALID_ROLES.includes(role_category)) {
        throw new AppError(`Invalid role_category. Must be one of: ${VALID_ROLES.join(", ")}`, 400);
    }
    if (difficulty && !["easy", "medium", "hard"].includes(difficulty)) {
        throw new AppError("Invalid difficulty. Must be one of: easy, medium, hard", 400);
    }
...
const submitAttemptToDB = async (userId, payload) => {
...
const getStatsFromDB = async (userId) => {
    ...
    const accuracy_percent = total_attempted === 0
        ? 0
...
const getAttemptHistoryFromDB = async (userId) => {
    const attempts = await prisma.quizAttempts.findMany({
        where: { user_id: userId },
