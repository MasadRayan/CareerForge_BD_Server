...
export const usageQuota = async (req, res, next) => {
    const user = req.user;
    ...
    try {
        ...
        if (quota.analyses_used_this_month >= FREE_TIER_MONTHLY_LIMIT) {
            sendResponse(res, 429, false, `Free tier limit reached. You can only run ${FREE_TIER_MONTHLY_LIMIT} analysis per month. Upgrade to premium for unlimited access.`);
            return;
        }
...
//# sourceMappingURL=usagesQuotas.js.map