...
import type { ReadinessScoreResponse, ReadinessHistoryItem, SubScores } from "./readiness.interface.js";

const getAtsComponent = async (userId: string): Promise<number> => {
...
const getRoadmapComponent = async (userId: string): Promise<number> => {
...
const getInterviewComponent = async (userId: string): Promise<{ score: number; sub_scores: SubScores }> => {
  ...
  if (behavioralAnswers.length > 0) {
    ...
    for (const a of behavioralAnswers) {
      const feedback = a.ai_feedback as Record<string, unknown> | null;
...
const calculateScore = async (userId: string): Promise<ReadinessScoreResponse> => {
  ...
  const composite = Math.round(
    ats * 0.35 + roadmap * 0.35 + interview.score * 0.30,
  );
...
const getHistory = async (userId: string): Promise<ReadinessHistoryItem[]> => {
