...
export const sendStudyReminder = async (userId: string): Promise<void> => {
  const user = await prisma.users.findUnique({
    where: { id: userId },
    select: { name: true, email: true },
  });
...
export const sendSubscriptionExpiry = async (
  userId: string,
  daysLeft: number,
): Promise<void> => {
  const user = await prisma.users.findUnique({
  ...
  await sendEmail(
    user.email,
    `Subscription expiring in ${daysLeft} day(s) — CareerForge BD`,
    html,
  );
};
